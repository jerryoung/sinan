...
原有 `qmt_rpc` 留在 `settings.yaml`。它描述本机连接 QMT RPC 的网络参数，
服务数据源、监控等系统能力，不是某个策略的下单参数，不随实盘配置复制。

## 设置界面

“设置”页拆成两个页签：

1. **系统设置**：保留本金、执行、风控、数据源、通知、`qmt_rpc` 和高级 YAML。
