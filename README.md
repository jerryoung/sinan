...
| `donchian` | 55 日突破 + 3×ATR 吊灯止损(海龟慢系统变体) | 基准 |
| `livermore` | 《股票作手操盘术》市场要诀状态机 + 试探加码 | 低换手互补槽(~1 次/年) |
| `ma_cross` / `tsmom` | EMA20/60 双均线 / 12-1 时序动量 | 对照组 |
| `supertrend` | ATR 棘轮通道 | 验证过,与海龟同源(ρ=0.86),不入组合 |
| `rebalance` | 等权持有 + 周期再平衡(M/Q/Y/N) | 懒人基准 |
| `cadence` | 任意策略降频包装器(周/月) | 实验结论:勿降频 |
...
- **策略调用**:引擎与 run_signal 都必须 `fn(ctx, **params, lookback=cfg.lookback)`;
